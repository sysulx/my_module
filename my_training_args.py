...
from transformers import TrainingArguments as T_TrainingArguments


@dataclass
class TrainingArguments(T_TrainingArguments):

    evaluate_trainset_during_training: bool = field(
        default=False, metadata={"help": "Run trainset evaluation during training at each logging step."}
    )
    eval_trainset_steps: int = field(default=1000, metadata={"help": "Run an evaluation on trainset every X steps."})
    trainset_sampling: int = field(default=100, metadata={"help": "sampling the trainset for fast evaluation, 0 means all train set"})
    
    """
    TrainingArguments is the subset of the arguments we use in our example scripts
    **which relate to the training loop itself**.

    Using :class:`~transformersing :class:`~transformers.HfArgumentParser` we can turn this class
